...
class EfficientNet_grapheme(nn.Module):
    def __init__(self, eff_version, pretrain):
        """
        eff_version (str): 0 - 7
        
        """
        super(EfficientNet_grapheme, self).__init__()
        if pretrain:
            eff_net = EfficientNet.from_pretrained(f"efficientnet-b{eff_version}")
        else:
            eff_net = EfficientNet.from_name(f"efficientnet-b{eff_version}")
        ...
        self._root_fc = nn.Linear(1792, 168)
        # self._root_swish = MemoryEfficientSwish()
        
        self._vowel_fc = nn.Linear(1792, 11)
        # self._vowel_swish = MemoryEfficientSwish()
        
        self._consonant_fc = nn.Linear(1792, 7)
        # self._consonant_swish = MemoryEfficientSwish()
